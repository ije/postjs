...
class AnsiUp {
    ...
    private _escape_for_html: boolean;

    private _csi_regex: RegExp;
    private _osc_st: RegExp;
    private _osc_regex: RegExp;

    private _url_whitelist: Record<string, any> = {};
    ...
    set url_whitelist(arg: Record<string, any>) {
        this._url_whitelist = arg;
    }

    get url_whitelist(): Record<string, any> {
...
// ES5 template string transformer
// Multi-Line On
function rgxG(tmpl: TemplateStringsArray) {
    // Use the 'raw' value so we don't have to double backslash in a template string
    let regexText: string = tmpl.raw[0];
